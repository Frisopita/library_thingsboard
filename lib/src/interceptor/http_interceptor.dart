...
class HttpInterceptor extends Interceptor {
  ...
  Future _refreshTokenAndRetry( DioException error, ErrorInterceptorHandler handler, InterceptorConfig config,
  ) async {
    try {
      await _tbClient.refreshJwtToken(
        internalDio: _internalDio,
        interceptRefreshToken: true,
      );
    } catch (e) {
      if (e is DioException && e.requestOptions.path.startsWith('/api/')) {
        _updateLoadingState(config, false);
      }
